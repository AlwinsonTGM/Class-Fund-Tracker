...
export function MobileBalanceToast({ balance }: MobileBalanceToastProps) {
  ...
  useEffect(() => {
    ...
    if (prev !== balance) {
      ...
      // Reset auto-dismiss timer on new update
  ...
  return (
    <div
      aria-live="polite"
      className="fixed bottom-20 right-3.5 z-50 sm:hidden max-w-[280px] w-auto animate-fade-slide-in pointer-events-auto"
    >
      <div className="flex items-center gap-3 rounded-2xl border border-primary/25 bg-foreground p-3.5 text-background shadow-2xl backdrop-blur-md">
        <div className={`flex size-9 shrink-0 items-center justify-center rounded-xl ${
          ...
          {delta && delta < 0 ? (
            ...
            <TrendingUp className="h-5 w-5 animate-bounce" style={{ animationDuration: '600ms', animationIterationCount: '2' }} />
          )}
        </div>

        <div className="flex flex-col min-w-0 pr-1">
          <div className="flex items-center gap-1.5 justify-between">
            <span className="text-[10px] font-bold uppercase tracking-wider text-background/60">Total Balance</span>
            {delta !== null && (
              <span className={`text-[11px] font-extrabold px-1.5 py-0.2 rounded ${
                delta > 0 ? 'bg-emerald-500/30 text-emerald-300' : 'bg-rose-500/30 text-rose-300'
              }`}>
                {formattedDelta}
              </span>
            )}
          </div>
          <p className="text-base font-bold tracking-tight text-background truncate">
            {formattedBalance}
          </p>
        </div>
        ...
        <button
          ...
          className="size-6 text-background/60 hover:text-background flex items-center justify-center rounded-full shrink-0 -mr-1"
