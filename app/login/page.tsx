...
function LoginForm() {
  ...
  if (error === 'Unauthorized') {
    displayError = 'Access Denied: This Google account is not pre-registered as an officer in Supabase.'
  }
    ...
    <main className="min-h-screen bg-background px-4 py-12 flex flex-col justify-center items-center sm:px-6 anim-fade-slide-in">
      <div className="w-full max-w-md flex flex-col gap-6">
        <header className="flex flex-col gap-2 text-center">
          ...
          <p className="text-pretty text-sm leading-6 text-muted-foreground sm:text-base">Officer Portal Login</p>
        </header>

        <section className="rounded-2xl border border-border bg-card p-6 shadow-lg sm:p-8">
          {displayError && (
            <div className="mb-6 p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-sm font-medium text-destructive leading-5">
              {displayError}
            </div>
          )}
    ...
    <Suspense
      fallback={
        <main className="min-h-screen bg-background px-4 py-12 flex flex-col justify-center items-center sm:px-6">
          <div className="w-full max-w-md flex flex-col gap-6 text-center text-muted-foreground">
